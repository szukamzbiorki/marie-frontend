...
var BugDispatch = {
    ...
    initialize: function (a) {
      ...
      ;-1 == this.modes.indexOf(this.options.mouseOver) &&
      ...
      if (this.transform) {
        ...
        for (c = 0; c < b; c++) {
          ...
          var e = SpawnBug()
          a.wingsOpen = this.options.canFly
            ? 0.5 < Math.random()
              ? !0
              : !1
            : !0
          a.walkSpeed = this.random(
    ...
    stop: function () {
      for (var a = 0; a < this.bugs.length; a++)
        this.spawnDelay[a] && clearTimeout(this.spawnDelay[a]),
          this.bugs[a].stop()
    },
    end: function () {
      for (var a = 0; a < this.bugs.length; a++)
    ...
    reset: function () {
      this.stop()
      for (var a = 0; a < this.bugs.length; a++)
        this.bugs[a].reset(), this.bugs[a].walkIn()
    },
    killAll: function () {
      for (var a = 0; a < this.bugs.length; a++)
        this.spawnDelay[a] && clearTimeout(this.spawnDelay[a]),
    ...
    check_if_mouse_close_to_bug: function (a) {
      if ((a = a || window.event)) {
        ...
        for (d = 0; d < a; d++) {
          ...
          e &&
            Math.abs(e.top - c) + Math.abs(e.left - b) <
              this.options.eventDistanceToBug &&
    ...
    on_bug: function (a) {
      if (a.alive) {
        ...
        'random' === b &&
          (b = this.modes[this.random(0, this.modes.length - 1, !0)])
        if ('fly' === b) a.stop(), a.flyRand()
          ...
          if ('flyoff' === b) a.stop(), a.flyOff()
            ...
            'multiply' === b &&
            !this.multiplyDelay &&
            this.bugs.length < this.options.maxBugs
          ) {
            var c = SpawnBug()
            b = JSON.parse(JSON.stringify(this.options))
            var d = a.getPos(),
              e = this
            b.wingsOpen = this.options.canFly
              ? 0.5 < Math.random()
                ? !0
                : !1
              : !0
            b.walkSpeed = this.random(
              this.options.minSpeed,
              this.options.maxSpeed
            )
...
var Bug = {
    ...
    stop: function () {
      ...
      this.flyperiodical &&
        (clearTimeout(this.flyperiodical), (this.flyperiodical = null))
      this.twitchTimer &&
        (clearTimeout(this.twitchTimer), (this.twitchTimer = null))
    },
    remove: function () {
      this.active = !1
      this.inserted &&
        this.bug.parentNode &&
        (this.bug.parentNode.removeChild(this.bug), (this.inserted = !1))
    },
    reset: function () {
      ...
      if (this.animating && this.alive && this.active) {
        ...
        if (
          ...
          0 >= --this.large_turn_counter &&
            ((this.large_turn_angle_deg = this.random(
              1,
              this.options.maxLargeTurnDeg,
              !0
            )),
          ...
          if (0 >= --this.small_turn_counter)
            ...
            if (
              (0 < this.large_turn_angle_deg && 0 > a) ||
              (0 > this.large_turn_angle_deg && 0 < a)
            )
              a = -a
          ...
          this.moveBug(
            this.bug.left +
              (c / 100) * this.options.walkSpeed * Math.cos(this.angle_rad),
            this.bug.top +
              (c / 100) * this.options.walkSpeed * -Math.sin(this.angle_rad),
            90 - this.angle_deg
          )
          this.walkFrame()
        }
    ...
    makeBug: function () {
      if (!this.bug && this.active) {
        ...
        b.className = 'bug'
        b.style.background =
          'transparent url(' + this.options.imageSprite + ') no-repeat 0 ' + a
    ...
    drawBug: function (a, b) {
      ...
      this.bug &&
        ...
        this.inserted ||
          ((this.inserted = !0), document.body.appendChild(this.bug)))
    },
    toggleStationary: function () {
      this.stationary = !this.stationary
      this.next_stationary()
      var a = this.wingsOpen ? '0' : '-' + this.options.bugHeight + 'px'
      this.bug.style.backgroundPosition = this.stationary
        ? '0 ' + a
        : '-' + this.options.bugWidth + 'px ' + a
    },
    walkFrame: function () {
    ...
    fly: function (a) {
      ...
      50 > Math.abs(d) + Math.abs(e) &&
        (this.bug.style.backgroundPosition =
          -2 * this.options.bugWidth +
          'px -' +
          2 * this.options.bugHeight +
          'px')
      30 > Math.abs(d) + Math.abs(e) &&
        (this.bug.style.backgroundPosition =
          -1 * this.options.bugWidth +
          'px -' +
          2 * this.options.bugHeight +
          'px')
      if (10 > Math.abs(d) + Math.abs(e))
        (this.bug.style.backgroundPosition = '0 0'), this.stop(), this.go()
      else {
        var g = Math.cos(f) * this.options.flySpeed
        f = Math.sin(f) * this.options.flySpeed
        if ((c > a.left && 0 < g) || (c > a.left && 0 > g))
          (g *= -1), Math.abs(d) < Math.abs(g) && (g /= 4)
        if ((b < a.top && 0 > f) || (b > a.top && 0 < f))
          (f *= -1), Math.abs(e) < Math.abs(f) && (f /= 4)
    ...
    dropping: function (a, b, c, d, e) {
      ...
      f >= c
        ? ((f = c),
          ...
          (this.angle_rad = this.deg2rad(this.angle_deg)),
          this.transform(
            'rotate(' + (90 - this.angle_deg) + 'deg) scale(' + this.zoom + ')'
          ),
          (this.bug.style.top = null),
          (this.bug.style.bottom =
            Math.ceil(
              (this.options.bugWidth * this.zoom -
                this.options.bugHeight * this.zoom) /
                2 -
                (this.options.bugHeight / 2) * (1 - this.zoom)
            ) + 'px'),
          (this.bug.style.left = this.bug.left + 'px'),
          (this.bug.style.backgroundPosition =
            '-' + 2 * e * this.options.bugWidth + 'px 100%'),
          this.twitch(e))
